...
class NotificationService(
    ...
    fun sendNotificationToUser(usuarioId: Int, titulo: String, cuerpo: String, data: Map<String, String> = emptyMap()) {
        ...
        // Guarda general de "no molestar": ningun usuario (propietario o cuidador) con este
        // indicador activo debe recibir notificaciones push, sin importar el flujo que las dispare.
        // NOTA: hoy no existe un broadcast a todos los cuidadores cuando se crea una solicitud
        // abierta (solo notificaciones dirigidas dueno<->cuidador via applications); si ese
        // broadcast se agrega en el futuro, tambien debe filtrar por noMolestar antes de llamar aqui.
        if (user?.noMolestar == true) {
            logger.info("Usuario {} tiene activado 'no molestar': se omite el envio de notificacion push", usuarioId)
            return
        }
