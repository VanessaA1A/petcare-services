...
export function mapDbRoleToApi(dbRole?: string | null): ApiRole {
  ...
  // fuzzy checks
  if (r.includes('gest') || r.includes('owner') || r.includes('propiet')) return 'OWNER';
  if (r.includes('cuid') || r.includes('care') || r.includes('client')) return 'CAREGIVER';
