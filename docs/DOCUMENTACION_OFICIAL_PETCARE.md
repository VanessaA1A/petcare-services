...
7. [Funcionalidades Completas](#7-funcionalidades-completas)
8. [Restricciones de Usuario](#8-restricciones-de-usuario)
9. [Decisiones de Diseño](#9-decisiones-de-diseño)
10. [Limitaciones Conocidas](#10-limitaciones-conocidas)
11. [Roadmap Futuro](#11-roadmap-futuro)
12. [Soporte y FAQ](#12-soporte-y-faq)
...
[sección 8](#8-restricciones-de-usuario): un usuario tiene un solo rol, los cuidadores no
...
2. **Elegir rol**: la primera vez que se abre la app, se elige "Soy dueño de perritos" o
...
1. **Panel de administración**: `http://<backend>/admin/dashboard` — cards con usuarios
...
2. **Gestión de usuarios**: `GET/PUT/DELETE /api/users/{id}` (vía Swagger o llamadas directas
   ...
   [Limitaciones Conocidas](#10-limitaciones-conocidas)).
...
```
┌─────────────────┐      HTTPS/JSON      ┌──────────────────────┐      JDBC      ┌──────────────┐
│   PetCareApp     │ ───────────────────► │   petcare-services     │ ─────────────► │  PostgreSQL   │
│ (Jetpack Compose)│ ◄─────────────────── │  (Spring Boot 3 / JVM) │ ◄───────────── │ (petcare-bd)  │
└─────────┬────────┘      WebSocket       └──────────┬───────────┘                └──────────────┘
          │                                            │
          │                                            ├── Firebase Cloud Messaging (push)
          │                                            └── Nominatim (geocodificación)
          └── Google Maps SDK (seguimiento en vivo)
```
...
5. La mayoría de endpoints de negocio (`/api/pets/**`, `/api/service-requests/**`, etc.) están
   ...
   manda el cliente, patrón usado en todo el proyecto); solo `/api/usuarios/me/**` exige un JWT
   válido vía `@PreAuthorize("isAuthenticated()")`.
...
```bash
psql -U postgres -c "CREATE DATABASE petcare;"
psql -U postgres -d petcare -f petcare-bd/database/schema.sql
psql -U postgres -d petcare -f petcare-bd/database/seeds/seeds_demo.sql   # datos de demo (opcional)
```
...
- **¿Por qué PetCare es solo para perros?** Enfocar el producto en una sola especie simplifica
  el alcance (razas, tamaños, necesidades de cuidado) para un proyecto universitario, y evita la
...
- El expediente médico solo es visible para el dueño de la mascota en la app — no existe aún
  una pantalla de detalle de mascota para que el cuidador lo vea durante un servicio activo.
